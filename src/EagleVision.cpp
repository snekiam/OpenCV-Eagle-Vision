...
int main()
{
	...
	cv::inRange( goalImage, Scalar( 56, 202, 23 ), Scalar( 81, 255, 206 ), goalImage );//threshold the HSV colortype
...
//		cv::imshow( "contours", drawing);
		cout<<"\nContour of Interest: "<<contourOfInterest<<"\n";
		if(contourOfInterest != -1){
			Rect rect = boundingRect(contours[contourOfInterest]);
			distance = (4.4 * 14 * 480)/(rect.height * 3);//focal length (mm) * height (inches) * photo hieght(pixels)/(obj height(pixels)*vertical sensor size(mm))
			cout <<"dist"<<distance<<"\n";
			centerPoint = Point((rect.x + rect.width/2), (rect.y + rect.height/2));
			float imageCntr = 640/2-.5;
			cout << "center point " <<centerPoint.x<<","<<centerPoint.y;
			focalLength = (0.5 * 640 / tan(74/2));
